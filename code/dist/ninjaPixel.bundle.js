        ...
        HorizontalBarChart.prototype.plot = function (_selection, barHeight) {
            ...
            _selection.each(function (_data) {
                ...
                if (_this._isTimeseries) {
                    ...
                    _this._yScale = d3.time.scale()
                        .domain([minY, maxY])
                        .range([0 + barH, _this._chartHeight - barH]);
                }
                else {
                    _this._yScale = d3.scale.ordinal()
                        .domain(_data.map(function (d, i) {
                        return d.y;
                    }))
                        .rangeRoundBands([0, _this._chartHeight], 0.1);
                }
                _this._xScale = d3.scale.linear()
                    .domain([minData, maxData])
                    .range([0, _this._chartWidth]);
                _this._barScale = d3.scale.linear()
                    .domain([Math.abs(maxData - minData), 0])
        ...
        GroupedBarChart.prototype.plot = function (_selection, barWidth) {
            ...
            _selection.each(function (_data) {
                ...
                if (_this._isTimeseries) {
                    ...
                    _this._xScale = d3.time.scale()
                        .domain([minX, maxX])
                        .range([0 + barW, _this._chartWidth - barW]);
                }
                else {
                    _this._xScale = d3.scale.ordinal()
                        .domain(_data.map(function (d, i) {
                        return d.x;
                    }))
                        .rangeRoundBands([0, _this._chartWidth], 0.1);
                }
                _this._yScale = d3.scale.linear()
                    .domain([minData, maxData])
                    .range([_this._chartHeight, 0]);
                _this._barScale = d3.scale.linear()
                    .domain([Math.abs(maxData - minData), 0])
        ...
        BubbleChart.prototype.plot = function (_selection) {
            ...
            _selection.each(function (_data) {
                ...
                if (_this._xMin) {
                    ...
                    minX = d3.min(_data, function (d) { return d.x; });
                }
                if (_this._xMax) {
                    maxX = _this._xMax;
                }
                else {
                    maxX = d3.max(_data, function (d) { return d.x; });
                }
                if (_this._y1Min) {
                    minY = _this._y1Min;
                }
                else {
                    minY = d3.min(_data, function (d) { return d.y; });
                }
                if (_this._y1Max) {
                    maxY = _this._y1Max;
                }
                else {
                    maxY = d3.max(_data, function (d) { return d.y; });
                }
                minR = d3.min(_data, function (d) { return d.r; });
                maxR = d3.max(_data, function (d) { return d.r; });
                ...
                bubbles.enter().append('circle')
                    ...
                    d3.select(this)
                        .style({
                        opacity: function (d, i) { return functor(itemOpacity, d, i); },
                        stroke: function (d, i) { return functor(itemStroke, d, i); }
                    });
                    myToolTip.hide();
                    onMouseout(d);
                })
...
(function (ninjaPixel) {
    var LineChart = (function (_super) {
        ...
        function LineChart() {
            ...
            _this._lineInterpolation = 'basis';
...
(function (ninjaPixel) {
    var Histogram = (function (_super) {
        ...
        function Histogram() {
            ...
            _this._histogramFunction = d3.layout.histogram();
            return _this;
        }
        ...
        Lollipop.prototype.plot = function (_selection) {
            ...
            _selection.each(function (_data) {
                ...
                bubbles.transition()
                    ...
                    opacity: function (d, i) { return functor(itemOpacity, d, i); },
                    stroke: function (d, i) { return functor(itemStroke, d, i); },
                    fill: function (d, i) { return functor(_this._headFill, d, i); }
                })
                    .attr({
                    cx: function (d) {
                        return xScale(d.x);
